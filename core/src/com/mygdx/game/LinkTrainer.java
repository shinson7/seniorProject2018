...
//        System.out.println("Working Directory = " + System.getProperty("user.dir")); //Checks the current directory you're in, for reference
    ...
    public void render() {
        ...
        if (currentScenario.initialExplainationPassed == false && prefaceDone) {
            ...
            font.draw(batch, currentScenario.ie + " Press ENTER to continue.", 125, 100, 400, 10, true);
            font.getData().setScale(.75f);
        } else if (currentScenario.videoPrefacePassed == false && currentScenario.initialExplainationPassed && dn == false) {
            font.draw(batch, currentScenario.vp + " Press ENTER to play the video.", 125, 100, 400, 10, true);
            font.getData().setScale(.75f);
        } else if (currentScenario.initialVideoPassed == false && currentScenario.videoPrefacePassed && currentScenario.initialExplainationPassed && dn == false) {
        ...
        } else if (currentScenario.initialExplainationPassed && currentScenario.choicePassed && currentScenario.finalExplaination1Passed == false && dn == false) {
            //show fe1
            font.draw(batch, currentScenario.fe1 + " Please press ENTER to find out why the other options were wrong.", 125, 100, 400, 10, true);
            font.getData().setScale(.5f);
        } else if (currentScenario.initialExplainationPassed && currentScenario.choicePassed && currentScenario.finalExplaination1Passed && currentScenario.finalExplaination2Passed == false && dn == false) {
            //show fe2
            font.draw(batch, currentScenario.fe2 + "Please press ENTER to watch the video of this being executed. When you have watched the video please press ENTER again to advance to the next scenario.", 125, 100, 400, 10, true);
            font.getData().setScale(.5f);
    ...
    public ArrayList initializeScenarios() {
        ...
        Scenario s3 = new Scenario( /*This is the endzone play*/
                ...
                /*Option 1*/ "Option 1: Make a jab step, take three hard steps to the break side, and then quickly burst to the open side. If you pull this cut off, the handler will have a very easy time getting it to you. Press 1 to select this answer.",
                /*Option 2*/ "Option 2: Cut directly towards the handler. Although you will drive into your defender, this could set you up for a continuation cut away from the handler. Press 2 to select this answer.",
                /*Option 3*/ "Option 3: Make a few jab steps and then run hard to the open space on the break side. Trust that your handler will have the throws to get around their mark. Press 3 to select this answer.",
                /*Option 4*/ "Option 4: Stay where you are. This gives others a chance to cut, and allows you to act as a continual anchor for the stack so that players do not get out of position. Press 4 to select this answer.",
                /*Correct option*/ 1,
                /*Action video*/ "video/endzoneAction.mp4",
